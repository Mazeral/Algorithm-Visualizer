...
const needsStartNode = computed(() =>
  ["BFS", "DFS", "Dijkstra", "Prim"].includes(selectedAlgorithm.value),
);
...
const layoutConfig = reactive({
  // Configuration for force layout
  iterations: 100,
...
watch(
  graphConfig,
  () => {
    if (isGraphAlgorithm.value) resetVisualization();
  },
  { deep: true },
);
watch(treeDepth, () => {
...
function ensureConnectivity(createdEdges) {
  ...
  while (disconnectedNodes.length > 0 && visitedArray.length > 0) {
    ...
    const connectToNode = nodes.find(
      (n) =>
        n.id === visitedArray[Math.floor(Math.random() * visitedArray.length)],
    );
    ...
    if (nodeToConnect && connectToNode) {
      ...
      console.error(
        "Could not find nodes to connect for ensuring connectivity.",
      );
      break;
    }
  }
...
function applyForceLayout() {
  ...
  for (let iter = 0; iter < iterations; iter++) {
    ...
    edges
      ...
      .forEach((edge) => {
        // Process each conceptual edge once
...
function calculateTreeLayout() {
  ...
  for (let depth = 0; depth < maxDepth - 1; depth++) {
    ...
    for (let i = 0; i < levelNodes; i++) {
      ...
      const leftChildIndex = nodes.findIndex(
        (n) => n.depth === depth + 1 && Math.floor(n.id / 2) === parentNode.id,
      ); // Find actual children by ID pattern if possible or index
        ...
        leftChildIndex !== -1
          ? nodes.findIndex(
              (n, idx) =>
                idx > leftChildIndex &&
                n.depth === depth + 1 &&
                Math.floor(n.id / 2) === parentNode.id,
            )
          : -1;
...
<template>
  <div class="main-container">
    <div class="algorithm-panel">
      ...
      <div
        v-if="needsStartNode || needsEndNode"
        class="config-section node-selection"
      >
        <h4>Node Selection</h4>
        <button
          v-if="needsStartNode"
          @click="selectStartNode"
        ...
        <button
          v-if="needsEndNode"
          @click="selectEndNode"
          :disabled="isRunning || selectingEnd"
          :class="{ selecting: selectingEnd }"
        >
          {{ endNode !== null ? `End: ${endNode}` : "Select End Node" }}
          {{ selectingEnd ? "..." : "" }}
        </button>
        <p v-if="selectingStart || selectingEnd" class="selection-hint">
          Click on a node in the visualization.
        </p>
      </div>

      <div class="config-section layout-config" v-if="isGraphAlgorithm">
        <h4>Layout Options</h4>
        <label title="Higher values push nodes apart more strongly">
          Repulsion:
          <input
            type="range"
            v-model.number="layoutConfig.repulsion"
            min="500"
            max="10000"
            step="100"
            :disabled="isRunning"
          />
          {{ layoutConfig.repulsion }}
        </label>
        <label title="Higher values make edges act like stiffer springs">
          Stiffness:
          <input
            type="range"
            v-model.number="layoutConfig.stiffness"
            min="0.01"
            max="0.2"
            step="0.01"
            :disabled="isRunning"
          />
          {{ layoutConfig.stiffness }}
        </label>
        <label
          title="Lower values make the layout settle faster but might overshoot"
        >
          Damping:
          <input
            type="range"
            v-model.number="layoutConfig.damping"
            min="0.1"
            max="0.95"
            step="0.05"
            :disabled="isRunning"
          />
          {{ layoutConfig.damping }}
        </label>
        <label>
          Iterations:
          <input
            type="range"
            v-model.number="layoutConfig.iterations"
            min="20"
            max="300"
            step="10"
            :disabled="isRunning"
        ...
        <button
          ...
          :disabled="isRunning || !isGraphAlgorithm"
        >
          Re-Apply Layout
        </button>
      </div>
      ...
      <div class="main-controls">
        <button
          ...
          :disabled="
            isRunning ||
            (needsStartNode && startNode === null) ||
            (needsEndNode && endNode === null)
          "
		class="start-button"
        >
    ...
    <div class="visualization-container">
      ...
      >
        ...
        <g class="edge-weights">
          <text
            v-for="edge in edges.filter((e) => e.from < e.to)"
...
body {
  ...
  font-family:
    ...
    Arial, sans-serif; /* Modern font stack */
...
.node.start {
  fill: var(--color-node-start) !important;
  stroke: darkgreen !important;
}
.node.end {
  fill: var(--color-node-end) !important;
...
.edge.visited {
  ...
  stroke-width: var(--edge-visited-stroke-width);
}
.edge.in-path {
  stroke: var(--color-edge-path);
  stroke-width: var(--edge-path-stroke-width);
}
